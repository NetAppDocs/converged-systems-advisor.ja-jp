...
.  accounts on FlexPod devices,各 FlexPod コンポーネントでアカウントを作成します。
+
...
TACACS+ サーバを使用していて、 CSA ユーザ権限を付与する必要がある場合は、に進みます  CSA user privileges using a TACACS+ server。
...
.手順
.  and installing the agent,エージェントをダウンロードしてインストールします
.  up networking for the agent,エージェントのネットワークを設定します
.  an SSL certificate on the agent,必要に応じて、エージェントに SSL 証明書をインストールします
.  the agent to discover your FlexPod infrastructure,FlexPod インフラストラクチャを検出するようにエージェントを設定します
